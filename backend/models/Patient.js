...
patientSchema.pre("save", async function (next) {
  ...
  if (!this.patientId) {
    try {
      ...
      // Generate patient ID: PAT-XXXX (where XXXX is the sequence number)
      const sequenceNumber = String(sequence.value).padStart(4, "0");

      this.patientId = `PAT-${sequenceNumber}`;
    } catch (error) {
      return next(error);
    }
  }
