...
function OPDBillingCashMemo() {
  ...
  const [formData, setFormData] = useState({
    ...
    package: "",
    account: "",
    cashMemoDate: new Date().toISOString().split("T")[0],
    modeOfPayment: "Card",
    referenceNo: "",
    cashMemoNote: "",
    ...
    try {
      ...
      if (response.success && response.data?.patient) {
        ...
        if (billingResponse.success) {
          ...
          const balanceToPay =
            (billingData.summary?.overall?.pending || 0) > 0
              ? billingData.summary.overall.pending
              : 0;
    ...
    setServiceLines(
      serviceLines.map((line) => {
        if (line._id === id) {
          ...
          const total = (updated.amount * updated.qty) - updated.discount + updated.incentive;
  ...
  const handleSave = async (print: boolean = false) => {
    ...
    if (serviceLines.length === 0) {
      showError("Please add at least one service line");
      return;
    }
    ...
    try {
      ...
      // Create OPD record with cash memo data
      const opdData = {
        patientId: resolvedPatientId,
        doctorId: formData.doctorId,
        visitDate: formData.cashMemoDate,
        consultationFee: 0,
        additionalCharges: calculateSubtotal(),
        discount: parseFloat(formData.overallDiscountValue) || 0,
        totalAmount: calculateTotal(),
        paidAmount: calculateTotal(), // Cash memo is paid immediately
        paymentStatus: "paid",
        paymentMethod: formData.modeOfPayment.toLowerCase(),
        paymentDate: formData.cashMemoDate,
        notes: formData.cashMemoNote,
        // Additional fields for cash memo
        billingDocumentType: "cash_memo",
        cashMemoNumber: `CM-${Date.now()}`,
      ...
      if (response.success) {
        showSuccess("Cash memo created successfully");
        if (print) {
        ...
        showError(response.message || "Failed to create cash memo");
      }
    } catch (err: any) {
    ...
    <div className="flex min-h-screen bg-slate-50">
      ...
      <div className="flex flex-1 flex-col sidebar-content-margin">
        <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur-sm shadow-sm">
          <div className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-4 lg:px-8">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold text-slate-900 sm:text-2xl">Add Outpatient Cash Memo</h1>
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
          ...
          {/* Cash Memo Details */}
          <div className="mb-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h3 className="mb-4 text-lg font-semibold text-slate-900">Cash Memo Details</h3>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                  Cash Memo Date <span className="text-red-500">*</span>
                </label>
                <div className="relative">
                  <input
                    type="date"
                    value={formData.cashMemoDate}
                    onChange={(e) => {
                      setFormData({ ...formData, cashMemoDate: e.target.value });
                      // Update all service line dates
                      setServiceLines(
                        serviceLines.map((line) => ({ ...line, date: e.target.value }))
                      );
                    }}
                    className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                  />
          ...
          <div className="mb-6 rounded-xl border border-slate-200 bg-white shadow-sm">
            <div className="bg-indigo-600 px-6 py-3">
              <div className="grid grid-cols-8 gap-2 text-xs font-semibold text-white">
                <div>Date</div>
                <div>Services</div>
                <div>Description</div>
                <div>Incentive</div>
            ...
            <div className="p-6">
              {serviceLines.length === 0 ? (
                ...
                <div className="space-y-2">
                  {serviceLines.map((line) => (
                    <div key={line._id} className="grid grid-cols-8 gap-2 items-center border-b border-slate-200 pb-2">
                      <input
                        type="date"
                        value={line.date}
                      ...
                      <input
                        ...
                        onChange={(e) => handleUpdateServiceLine(line._id, "incentive", parseFloat(e.target.value) || 0)}
                        placeholder="0"
                        className="rounded border border-slate-300 px-2 py-1 text-xs"
                      />
                      <input
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Write Cash Memo Note</label>
                    <textarea
                      value={formData.cashMemoNote}
                      onChange={(e) => setFormData({ ...formData, cashMemoNote: e.target.value })}
                      rows={6}
                      className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200"
...
export default OPDBillingCashMemo;
