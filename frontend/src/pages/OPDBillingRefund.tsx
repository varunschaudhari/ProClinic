...
import { opdAPI, patientsAPI } from "../utils/api";
...
function OPDBillingRefund() {
  ...
  const fetchPatientAndBills = async () => {
    try {
      ...
      const billsRes = await opdAPI.getAll({ patientId: patientIdParam as string });
      const records = billsRes?.data?.opdRecords || [];
      const eligible = records.filter((r: any) => (r.paidAmount || 0) > 0);
      setPaidBills(eligible);
      ...
      if (eligible.length > 0) {
        ...
        setFormData((prev) => ({
          ...prev,
          billId: first._id,
          billNumber: first.opdNumber,
          amount: Math.min(first.paidAmount || 0, first.totalAmount || 0).toString(),
        }));
      }
    } catch (err) {
      console.error(err);
  ...
  const selectedBill = useMemo(
    () => paidBills.find((b) => b._id === formData.billId) || null,
    [paidBills, formData.billId]
  );

  const maxRefundAmount = selectedBill ? Number(selectedBill.paidAmount || 0) : 0;
  ...
  const handleSave = async (print: boolean) => {
    ...
    if (amountNum > maxRefundAmount) {
      showError("Refund amount cannot be more than paid amount");
      return;
    }

    try {
      setSaving(true);
      const res = await opdAPI.refund(formData.billId, {
        amount: amountNum,
        method: formData.modeOfPayment.toLowerCase(),
        referenceNo: formData.referenceNo || undefined,
        note: formData.note || undefined,
        refundedAt: formData.refundDate,
      });
      ...
      if (res.success) {
        ...
        showError(res.message || "Failed to process refund");
      }
    } catch (err: any) {
        ...
        <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur-sm shadow-sm">
          <div className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-4 lg:px-8">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold text-slate-900 sm:text-2xl">Add Refund</h1>
            </div>
          </div>
        </header>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div className="lg:col-span-2 space-y-6">
              <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <h3 className="mb-4 text-lg font-semibold text-slate-900">Refund Details</h3>
                <div className="space-y-4">
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const bill = paidBills.find((b) => b._id === e.target.value);
                          setFormData((prev) => ({
                            ...prev,
                            billId: e.target.value,
                            billNumber: bill?.opdNumber || "",
                            amount: bill ? String(bill.paidAmount || 0) : "",
                          }));
                        }}
                        className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                      >
                        ...
                        {paidBills.map((b) => (
                          <option key={b._id} value={b._id}>
                            {b.opdNumber} — Paid ₹{Number(b.paidAmount || 0).toFixed(2)}
                          </option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      {selectedBill && (
                        <p className="mt-1 text-xs text-slate-500">
                          Max refundable: ₹{Number(maxRefundAmount).toFixed(2)}
                        </p>
                      )}
                    </div>
                    <div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Refund Note</label>
                    <textarea
                      value={formData.note}
                      onChange={(e) => setFormData({ ...formData, note: e.target.value })}
                      rows={6}
                      className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                      placeholder="Reason / note for refund..."
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-slate-900">{formData.billNumber || "-"}</span>
                </div>
                <div className="flex justify-between">
...
export default OPDBillingRefund;
